  ...
  def index
    ...
    if params[:op] == 'search'
      ...
      if @platform_name.blank?
        ...
        @player1 = OnlinePlayer.gaming_platform_id_is(@platform.id).username_is(@player1_name).first
        @player2 = OnlinePlayer.gaming_platform_id_is(@platform.id).username_is(@player2_name).first
      end
