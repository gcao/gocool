...
function addUploadField() {
  ...
  jQuery('#file_fields').append('<div id="' + fieldId + '" class="additionalUpload"><input name=' + fieldName + ' size="60" type="file" /><input class="btn btn-danger" type="button" value="Delete This" onclick="removeUploadField(\'' + fieldId + '\')"/></div>');
  jQuery('#' + fieldId).change(addUploadField);
}

function removeUploadField(id) {
  if (jQuery('#' + id + ' input[type=file]').val())
