class Uploader
  def process email, files
    files.map do |file|
      ...
      result.status = I18n.t('upload.status.success')
      upload = Upload.new(:email => email, :upload => file)
      if upload.valid?
        process_valid_upload upload, result
      else
        result.status = I18n.t('upload.status.validation_error')
  ...
  def process_valid_upload upload, result
    ...
    if found = process_duplicate_upload(upload)
      result.status = I18n.t('upload.status.already_uploaded')
      result.detail = I18n.t('upload.already_uploaded_detail')
      result.existing_upload = found
    else
      upload.save_game
    end
  rescue => e
    result.status = I18n.t('upload.status.error')
