 ...
 * Upgrade ec2 instance to Rails 3
 * Integrate with gnugo
 * Create special robot player, invitations are automatically accepted if opening
   game limit is not reached
 * Gnugo player can run on a different machine
 * Modify game model and record misc game properties in a separate table
 * Modify sgf_parser to fully conform to latest sgf spec
